load(file = "single_double_visit_abundance/raw_data/abunData_large.RData")
...
numIter <- 10 ## do smaller number of iterations because so computationally intensive
for (i in 1:numIter) {
  m <- 1:7
...
save(resultsLogRegSpline, basisP, basisPsi, file = "single_double_visit_abundance/results/dvAbundanceLarge.RData")

#### Figures S1h and S2h ####

load(file = "single_double_visit_abundance/raw_data/abunData_large.RData")
load("single_double_visit_abundance/results/dvAbundanceLarge.RData")

thin <- seq(1, n, by = 100)
jpeg("paper_figures/figS1h.jpeg", width = 824, height = 634, units = "px")
...
for (i in 1:25) {
  ...
  points((zData[[i]][idx])[thin], (fitted.val[idx])[thin], col = "red")
}
...
jpeg("paper_figures/figS2h.jpeg", width = 824, height = 634, units = "px")
...
save(resultsLeleSpline, basisPsi, basisP, file = "single_double_visit_abundance/results/svAbundanceLarge.RData")

#### Figures S1d and S2d ####

load(file = "single_double_visit_abundance/raw_data/abunData_large.RData")
load("single_double_visit_abundance/results/svAbundanceLarge.RData")

thin <- seq(1, 2 * n, by = 100)

jpeg("paper_figures/figS1d.jpeg", width = 824, height = 634, units = "px")
idx <- order(zData[[1]])
...
for (i in 1:25) {
  ...
  idx <- order(zData[[i]])
  lines((zData[[i]][idx])[thin], (fitted.val[idx])[thin], col = "red")
}
...
jpeg("paper_figures/figS2d.jpeg", width = 824, height = 634, units = "px")
idx <- order(mData[[1]])
...
i <- 1
idx <- order(mData[[i]])
lines(mData[[i]][idx], pData[[i]][idx], lwd = 1)
mtext(
  text = "p(z)",
  side = 2,
  line = 2.5, cex = 2
