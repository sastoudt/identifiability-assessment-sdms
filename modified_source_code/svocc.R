## modified https://github.com/psolymos/detect/blob/master/R/svocc.R
## call svocc.fit2 so that we get more information out of optim
## search MODIFY
svocc2 <-
            ...
            model = TRUE, x = FALSE, ...)
  {
    ...
    ## fit
    fit <- svocc.fit2(Y, X, Z, link.sta, link.det, penalized = penalized, auc = FALSE, 
                      method=method, inits=inits, ...) ## MODIFIED
    #browser()
    ## return value
    out <- c(fit, out)
    if (!model) 
