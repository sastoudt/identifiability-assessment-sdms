load(file = "single_double_visit/raw_data/svdvOccurData_largeNoSaturateLine.RData")
...
basisPsi <- basisP <- rstart <- negLL <- counts <- value <- isConverged <- vector("list", numParam)
...
save(resultsLogRegSpline, negLL, counts, value, isConverged, basisPsi, basisP, file = "single_double_visit/results/dvOccurSplineResults_largeNoSaturateLine.RData")

#### Figure 4h ####

load(file = "single_double_visit/results/dvOccurSplineResults_largeNoSaturateLine.RData")

thin <- seq(1, n, by = 100)

idx <- order(zData[[1]][1:n])
jpeg("paper_figures/fig4h.jpeg", width = 824, height = 634, units = "px")
...
jpeg("paper_figures/fig4p.jpeg", width = 824, height = 634, units = "px")
...
save(resultsLeleSpline, rstart, basisPsi, basisP, counts, loglik, isConverged, file = "single_double_visit/results/svOccurSplineResults_largeNoSaturateLine.RData")

#### Figure 4d ####

load(file = "single_double_visit/results/svOccurSplineResults_largeNoSaturateLine.RData")


thin <- seq(1, length(zData[[1]]), by = 100)
idx <- order(zData[[1]])
jpeg("paper_figures/fig4d.jpeg", width = 824, height = 634, units = "px")
...
idx <- order(mData[[1]])
jpeg("paper_figures/fig4l.jpeg", width = 824, height = 634, units = "px")
...
idx <- order(mData[[1]])
lines((mData[[1]][idx])[thin], (pData[[1]][idx])[thin])
