...
load(file = "presence_absence_only/raw_data/popaData_large.RData")
...
save(resultsLogRegSpline, basisMfull, file = "presence_absence_only/results/paSplineResults_largeMoreKnots.RData")


#### Figure 2h ####

load("presence_absence_only/results/paSplineResults_largeMoreKnots.RData")

thin <- seq(1, n, by = 100)
jpeg("paper_figures/fig2h.jpeg", width = 824, height = 634, units = "px")

plot(sort(covDataM[[1]][thin]), sort(0.5 * psiDataM[[1]][thin]), ylim = c(0, 1), type = "l", lwd = .5, xlab = "x", ylab = "", cex.axis = 2, cex.lab = 2, xlim = c(-2.8, 8))
for (i in 2:25) {
...
for (i in 1:numIter) {
  ...
  out <- tryCatch(optim(rep(0.1, 11), lik, hessian = F), error = function(i) {
    return(rep(NA, 11))
  })
  print(i)
...
save(resultsRoyleSpline, basisMfull, basisMDfull, file = "presence_absence_only/results/poSplineResults_largeMoreKnots.RData")

#### Figure 2d ####

load("presence_absence_only/results/poSplineResults_largeMoreKnots.RData")

thin <- seq(1, 2 * n, by = 100)
jpeg("paper_figures/fig2d.jpeg", width = 824, height = 634, units = "px")
plot(sort(covDataM[[1]][thin]), sort(0.5 * psiDataM[[1]][thin]), ylim = c(0, 1), type = "l", lwd = .5, xlab = "x", ylab = "", cex.axis = 2, cex.lab = 2, xlim = c(-2.5, 8))
for (i in 2:25) {
  lines(sort(covDataM[[i]][thin]), sort(0.5 * psiDataM[[i]][thin]), lwd = .5)
}
for (i in 1:25) {
