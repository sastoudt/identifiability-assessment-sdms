...
load(file = "presence_absence_only/raw_data/popaData_small.RData")
...
save(resultsLogRegSpline, basisMfull, file = "presence_absence_only/results/paSplineResults_smallMoreKnots.RData")

#### Figure 2f ####
load("presence_absence_only/results/paSplineResults_smallMoreKnots.RData")

thin <- seq(1, n, by = 1)
jpeg("paper_figures/fig2f.jpeg", width = 824, height = 634, units = "px")
plot(sort(covData[[1]][thin]), sort(0.5 * psiData[[1]][thin]), ylim = c(0, 1), type = "l", lwd = .5, xlab = "x", ylab = "", cex.axis = 2, cex.lab = 2)
for (i in 2:25) {
...
for (i in 1:25) {
  ...
  idx <- order(covData[[i]][1:n])
  lines((covData[[i]][idx])[thin], (fitted.val[idx])[thin], col = "red")
}

lines(sort(covData[[1]][thin]), sort(0.5 * psiData[[1]][thin]))
mtext(
...
save(resultsRoyleSpline, basisMfull, basisMDfull, file = "presence_absence_only/results/poSplineResults_smallMoreKnots.RData")

load("presence_absence_only/results/poSplineResults_smallMoreKnots.RData")

thin <- seq(1, 2 * n, by = 1)

#### Figure 2b ####
jpeg("paper_figures/fig2b.jpeg", width = 824, height = 634, units = "px")
...
for (i in 1:25) {
  ...
  idx <- order(covData[[i]])
  lines((covData[[i]][idx])[thin], (fitted.val[idx])[thin], col = "red", cex = .5)
}
